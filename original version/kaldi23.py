...
"""
#IDEAS FOR VERSIONS MOVING FORWARD:

#I want to add the roasting time to the profile database somewhere

#have the "run profile" function count down until the profile is done

#need to make the roast time from the stopwatch get stored in the profile file somewhere, so that 
#it can display in the "roast time" label at the top left on the Kaldi window.

the following keystrokes exist
control-return //this one 'jumps' the the blower and heate to whatever is in the fields
control-k //this stops the interval logging, and turns off the heater/blower
 - ideally the last thing the roaster hits before dumping the beans
"""

"""
MOST RECENT: the time seems to be mostly accurate, and the file saves as a CSV, so it can be loaded into excel easier.
"""
