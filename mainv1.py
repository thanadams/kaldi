...
temps = collections.deque(maxlen=1200) # Increased size for 2x sampling rate
ror_history = collections.deque(maxlen=600)
smoothed_bt_history = collections.deque(maxlen=1200) # Increased size for 2x sampling rate
intervals = []
roast_events = []
start = True
profile = False
...
def update_graph(ax, canvas):
    ...
    for event in roast_events:
        ...
        if ax.get_ylim()[1] > 0: # Ensure ylim is valid before placing text
...
def tempit():
    """
    Reads sensor data twice per second, calculates a smoothed bean temperature, and then calculates
    the Rate of Rise (RoR) once per second based on the smoothed data.
    """
    global temps, ror_history, smoothed_bt_history, timer
    ...
    # Store raw temps with Bean Mass (ch1) first
    temps.append([ch1, ch2, timetosend])
    
    # --- Step 1: Apply Rolling Average Filter to Bean Temp ---
    try:
        ...
        current_ror_window_seconds = 15 # Default to a safe value on error
    ...
    if len(temps) >= num_samples_for_window:
        # Get the last 'n' number of readings for the bean probe
        last_n_temps = [temps[i][0] for i in range(-num_samples_for_window, 0)]
        smoothed_bt = sum(last_n_temps) / len(last_n_temps)
        smoothed_bt_history.append(smoothed_bt)
    ...
    if len(smoothed_bt_history) % 2 == 0:
        ...
        if len(smoothed_bt_history) > num_samples_for_ror:
            # Get the current smoothed temp and the one from 30 seconds ago (60 samples ago)
            current_smoothed_bt = smoothed_bt_history[-1]
            prior_smoothed_bt = smoothed_bt_history[-(num_samples_for_ror + 1)]
...
def auto_mark_start_event():
    """Automatically marks the 'Start' event at 1 minute if not already marked."""
    global roast_events
    # Check if a 'Start' event has already been marked manually
    start_event_exists = any(event['event'] == 'Start' for event in roast_events)
    
    if state and not start_event_exists:
         print("Automatically marking 'Start' event at 1 minute.")
         mark_event("Start")

def swstart():
    global state, timer, after_id_auto_mark_start, roast_start_time, last_elapsed_time
    ...
    # If starting fresh
        ...
        last_elapsed_time = 0
        # Schedule the 'Start' event to be marked automatically at 1 minute
        if after_id_auto_mark_start:
            gui.after_cancel(after_id_auto_mark_start)
        after_id_auto_mark_start = gui.after(60000, auto_mark_start_event) # 60000 ms = 1 minute
...
def swreset():
    global timer, temps, ror_history, roast_events, smoothed_bt_history, after_id_auto_mark_start, roast_start_time, last_elapsed_time
    swpause()
...
def close_window(event=None):
    global after_id_update_graph, after_id_update_timeText, after_id_poll_sensors, after_id_poll_display, air_now, heat_now, after_id_auto_mark_start
